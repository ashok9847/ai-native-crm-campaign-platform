  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Account created successfully!", {
        description: "Your new workspace has been registered.",
      });
      // Redirect to the onboarding setup wizard since this is a new tenant
      router.push("/setup");
    } catch (err: any) {
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="bg-white border border-zinc-200 rounded-xl p-8 shadow-sm relative overflow-hidden">
          ...
          <div className="mt-6 text-center text-sm text-zinc-500 font-semibold">
            ...
            <Link href="/login" className="text-zinc-950 hover:text-zinc-800 font-bold transition-colors">
              Sign In
            </Link>
          </div>
        </div>
        ...
        <div className="text-center mt-6">
          <Link href="/" className="text-xs text-zinc-400 hover:text-zinc-650 transition-colors font-bold">
            ← Back to Home Page
          </Link>
        </div>
      </div>
